 ...
 * @method getSerial
 */
BlinkStick.prototype.getSerial = async function() {
  this.serial = await new Promise((resolve, reject) => {
    this.device.getStringDescriptor(3, (err, results) => {
 ...
 * @method close
 */
BlinkStick.prototype.close = function() {
  this.stop();
 ...
 */
BlinkStick.prototype.setColor = async function(red, green, blue, options) {
  const params = this.interpretParameters(red, green, blue, options);

  const [r, g, b] = [params.red, params.green, params.blue];

  if (params.options.channel === 0 && params.options.index === 0) {
    return this.setFeatureReport(1, [1, r, g, b]);
  }

  return this.setFeatureReport(5, [5, params.options.channel, params.options.index, r, g, b]);
};
 ...
 */
BlinkStick.prototype.setMode = function(mode) {
  return this.setFeatureReport(0x0004, [4, mode]);
};
 ...
 * @method getMode
 */
BlinkStick.prototype.getMode = async function() {
  try {
 ...
 *
 * @method getColor
 * @param {Number} [index] The index of the LED
 * @return {Promise<[Number, Number, Number]>} Callback returns three numbers: R, G and B [0..255].
 */
BlinkStick.prototype.getColor = async function(index = 0) {
  if (index === 0) {
    const buffer = await this.getFeatureReport(0x0001, 33);
    return buffer ? buffer.slice(0, 3) : [0, 0, 0];
  }

  const buffer = await this.getColors(index);
  return buffer ? [buffer[index * 3 + 1], buffer[index * 3], buffer[index * 3 + 2]] : [0, 0, 0];
};
 ...
 * @return {Promise<Array>} An array of LED data in the following format: [g0, r0, b0, g1, r1, b1...]
 */
BlinkStick.prototype.getColors = async function(count) {
  const { reportId, maxLeds } = _determineReportId(count * 3);
  const buffer = await this.getFeatureReport(reportId, maxLeds * 3 + 2);
  return buffer ? buffer.slice(2, buffer.length - 1) : undefined;
};
 ...
 */
BlinkStick.prototype.setColors = function(channel, data) {
  const params = _determineReportId(data.length);
  ...
  return this.setFeatureReport(params.reportId, report);
};
 ...
 * @return {Promise<String>} Hex string, eg "#BADA55".
 */
BlinkStick.prototype.getColorString = async function(index = 0) {
 ...
 */
async function getInfoBlock(device, location) {
  const buffer = await getFeatureReport(location, 33);
 ...
 */
function setInfoBlock(device, location, data) {
  var i,
  ...
  return setFeatureReport(location, buffer);
}
 ...
 */
BlinkStick.prototype.getInfoBlock1 = function() {
  return getInfoBlock(this.device, 0x0002);
};
 ...
 */
BlinkStick.prototype.getInfoBlock2 = function() {
  return getInfoBlock(this.device, 0x0003);
};
 ...
 */
BlinkStick.prototype.setInfoBlock1 = function(data) {
  return setInfoBlock(this.device, 0x0002, data);
};
 ...
 * @param {String} data Data value for InfoBlock
 */
BlinkStick.prototype.setInfoBlock2 = function(data) {
  return setInfoBlock(this.device, 0x0003, data);
};
 ...
 * @return {{ red: number, green: number, blue: number, options: object }} Contains sanitized RGB value, options and callback if they have been assigned
 */
BlinkStick.prototype.interpretParameters = function(red, green, blue, options) {
  var hex;
 ...
 */
BlinkStick.prototype.blink = function(red, green, blue, options) {
  ...
  var blinker = async (count) => {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(async () => {
        if (!self.animationsEnabled) return Promise.resolve();
 ...
 */
BlinkStick.prototype.morph = async function(red, green, blue, options) {
  const params = this.interpretParameters(red, green, blue, options);

  const duration = opt(params.options, 'duration', 1000);
  const steps = opt(params.options, 'steps', 25);

  const [cr, cg, cb] = await this.getColor(params.options.index);

  const morpher = async (count) => {
    if (!this.animationsEnabled) return Promise.resolve();

    const nextRed = cr + ((params.red - cr) / steps) * count;
    const nextGreen = cg + ((params.green - cg) / steps) * count;
    const nextBlue = cb + ((params.blue - cb) / steps) * count;
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (!this.animationsEnabled || count >= steps) return resolve();
 ...
 * @param {Object}   [options] additional options {"channel": 0, "index": 0, "duration": 1000, "steps": 50}
 */
BlinkStick.prototype.pulse = async function(red, green, blue, options) {
  const params = this.interpretParameters(red, green, blue, options);

  await this.morph(params.red, params.green, params.blue, params.options);

  return this.morph(0, 0, 0, params.options);
};
 ...
 */
BlinkStick.prototype.setFeatureReport = function(reportId, data) {
  return attempt(
    () =>
      new Promise((resolve, reject) => {
        this.device.controlTransfer(0x20, 0x9, reportId, 0, Buffer.from(data), function(err) {
...
function attempt(promiseFn, retries = 10, error = undefined) {
  return promiseFn().catch((e) => {
    if (retries <= 0) {
      throw error;
    }

    return attempt(promiseFn, retries - 1, error || e);
  });
}
 ...
 */
BlinkStick.prototype.getFeatureReport = function(reportId, length) {
  return attempt(
    () =>
      new Promise((resolve, reject) => {
        this.device.controlTransfer(0x80 | 0x20, 0x1, reportId, 0, length, (err, data) =>
